  ...
  config = mkIf cfg.enable {
    systemd.services.transmission-proxy = {
      ...
      serviceConfig.ExecStartPre = ''
          ${pkgs.stdenv.shell} -c "mkdir -p ${homeDir} ${settingsDir} ${fullSettings.download-dir} ${fullSettings.incomplete-dir} && chmod 770 ${homeDir} ${settingsDir} ${fullSettings.download-dir} ${fullSettings.incomplete-dir} && rm -f ${settingsDir}/settings.json && cp -f ${settingsFile} ${settingsDir}/settings.json && printf 'strict_chain\nproxy_dns\nremote_dns_subnet 224\n\n[ProxyList]\n' > ${settingsDir}/proxychains.conf && echo '${config.services.transmission-proxy.proxy.type}  ${config.services.transmission-proxy.proxy.ip} ${config.services.transmission-proxy.proxy.port}' >> ${settingsDir}/proxychains.conf"
      '';
