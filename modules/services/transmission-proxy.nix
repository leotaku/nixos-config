...
let
  cfg = config.services.transmission-proxy;
  ...
  config = mkIf cfg.enable {
    systemd.services.transmission-proxy = {
      ...
      serviceConfig.ExecStartPre = ''
          ${pkgs.stdenv.shell} -c "mkdir -p ${homeDir} ${settingsDir} ${fullSettings.download-dir} ${fullSettings.incomplete-dir} && chmod 770 ${homeDir} ${settingsDir} ${fullSettings.download-dir} ${fullSettings.incomplete-dir} && rm -f ${settingsDir}/settings.json && cp -f ${settingsFile} ${settingsDir}/settings.json && printf 'dynamic_chain\nproxy_dns\nremote_dns_subnet 10\ntcp_read_time_out 15000\ntcp_connect_time_out 8000\n#localnet 127.0.0.1\n\n[ProxyList]\n' > ${settingsDir}/proxychains.conf && echo '${config.services.transmission-proxy.proxy.type}\t${config.services.transmission-proxy.proxy.ip}\t${config.services.transmission-proxy.proxy.port}' >> ${settingsDir}/proxychains.conf"
      '';
      serviceConfig.ExecStart = "${pkgs.proxychains}/bin/proxychains4 -f ${settingsDir}/proxychains.conf ${pkgs.transmission}/bin/transmission-daemon -f --port ${toString config.services.transmission-proxy.port}";
